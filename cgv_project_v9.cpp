...
void display() {
  ...
  if (POW) {
    ...
    for (size_t i = 0; i < names.size(); ++i) {
      ...
      if (record && show) {
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        for (size_t i = 0; i < names.size() - 1;
             ++i) // for displaying the added transactions
        {
  ...
  drawtext("----------------------------------------BLOCK CHAIN - THE NEXT BIG "
           "THING----------------------------------------",
           -9.0, 9.0, 3);
...
int main(int argc, char **argv) {
  ...
  glutAddMenuEntry("CRYPTOGRAPHY", 8);
